...
uint8_t RV3028::readRegister(uint8_t addr)
{
	try
	{
		getBus()->syncWrite(I2CAddress(getAddress()), {addr});
		vector<uint8_t> data = getBus()->syncRead(I2CAddress(getAddress()), 1);
		return data.data()[0];
	}
...
bool RV3028::readMultipleRegisters(uint8_t addr, uint8_t *dest, uint8_t len)
{
	...
	{
		getBus()->syncWrite(I2CAddress(getAddress()), {addr});
		vector<uint8_t> data = getBus()->syncRead(I2CAddress(getAddress()), len);
